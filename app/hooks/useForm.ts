...
import { MessagesForm} from '../interfaces'
import {
   InitialFormErrors,
   initialFormErrors,
   InitialFormState,
...
export const useForm = (messages: MessagesForm, submitUrl: string,captchaRef:RefObject<ReCAPTCHA>) => {
   ...
   const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
      ...
      try {
         setResponseMessage('')
         setSending(true)
         const data = await baseApi.post(submitUrl, {
            formData: formState,
         })
